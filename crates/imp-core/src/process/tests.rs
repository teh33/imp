...
async fn process_start_drains_both_streams_and_repeated_wait() {
    ...
    let info = manager
        .start(shell_request(dir.path(), "printf out; printf err >&2; exit 7"))
        .await
        .unwrap();
    ...
    let output = manager
        ...
        .await
        .unwrap();
    assert!(output.chunks.iter().any(|chunk| chunk.stream == OutputStream::Stdout && chunk.text.contains("out")));
    assert!(output.chunks.iter().any(|chunk| chunk.stream == OutputStream::Stderr && chunk.text.contains("err")));
...
async fn stdin_write_and_write_after_exit_are_typed() {
    ...
    let manager = ProcessManager::new();
    let info = manager
        .start(shell_request(dir.path(), "IFS= read -r line; printf '%s' \"$line\""))
        .await
        .unwrap();
    manager.write(info.id, b"hello\n").await.unwrap();
    manager.wait(info.id).await.unwrap();
    let output = manager.read(info.id, OutputCursor::start(info.id), 100).await.unwrap();
    assert!(output.chunks.iter().any(|chunk| chunk.text == "hello"));
    assert!(matches!(manager.write(info.id, b"late").await, Err(ProcessError::NotRunning(id)) if id == info.id));
}
...
async fn timeout_and_repeated_stop_are_terminal() {
    ...
    assert_eq!(manager.stop(info.id, StopOptions::default()).await.unwrap(), exit);
}
...
async fn graceful_stop_and_cancel_are_classified() {
    let dir = tempfile::tempdir().unwrap();
    let manager = ProcessManager::new();
    let stopped = manager.start(shell_request(dir.path(), "exec sleep 60")).await.unwrap();
    let exit = manager.stop(stopped.id, StopOptions::default()).await.unwrap();
    assert!(exit.cancelled);
    let cancelled = manager.start(shell_request(dir.path(), "exec sleep 60")).await.unwrap();
...
async fn unknown_ids_and_cursor_mismatch_are_rejected() {
    ...
    assert!(matches!(manager.wait(unknown).await, Err(ProcessError::UnknownProcess(_))));
    let info = manager.start(shell_request(dir.path(), "true")).await.unwrap();
    assert!(matches!(manager.read(info.id, OutputCursor::start(unknown), 10).await, Err(ProcessError::CursorProcessMismatch)));
}
...
async fn required_isolation_fails_closed_and_summary_is_truthful() {
    ...
    assert!(matches!(manager.start(required).await, Err(ProcessError::IsolationUnavailable)));

    let info = manager.start(shell_request(dir.path(), "true")).await.unwrap();
    assert_eq!(info.enforcement.backend, "host");
    assert!(!info.enforcement.enforced.contains(&EnforcementControl::FilesystemIsolation));
    assert!(info.enforcement.preflight_validated.contains(&EnforcementControl::WorkingDirectoryValidated));
}
...
async fn pty_is_not_silently_downgraded() {
    ...
    assert!(matches!(manager.start(request).await, Err(ProcessError::UnsupportedCapability("pty"))));
}
